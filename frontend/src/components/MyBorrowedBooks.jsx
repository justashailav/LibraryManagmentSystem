...
export default function MyBorrowedBooks() {
  ...
  const openReadPopup = (id) => {
    const book = books.find((book) => book._id === id || book.bookId === id); // more flexible
      ...
      <main className="relative flex-1 p-6 pt-28">
        ...
        <header className="flex flex-col gap-3 sm:flex-row md:items-center">
          <button className={`relative rounded sm:rounded-tr-none sm:rounded-br-none sm:rounded-tl-lg sm:rounded-bl-lg text-center border-2 font-semibold py-2 w-full sm:w-72 ${filter==="returned"?"bg-black text-white border-black":"bg-gray-200 text-black border-gray-200 hover:bg-gray-300"}`} onClick={()=>setFilter("returned")}>
           ...
           <button className={`relative rounded sm:rounded-tl-none sm:rounded-bl-none sm:rounded-br-lg sm:rounded-tr-lg text-center border-2 font-semibold py-2 w-full sm:w-72 ${filter==="nonReturned"?"bg-black text-white border-black":"bg-gray-200 text-black border-gray-200 hover:bg-gray-300"}`} onClick={()=>setFilter("nonReturned")}>
            Non Returned Books
          </button>
        </header>

        {
          booksToDisplay && booksToDisplay.length>0 ?(
            ...
            <h3 className="text-3xl mt-5 font-medium">No nonReturned books found!</h3>
          )
        }
      </main>
